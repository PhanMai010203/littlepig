...
class BudgetUpdateServiceImpl implements BudgetUpdateService {
  ...
  final BehaviorSubject<List<Budget>> _budgetUpdatesController =
      BehaviorSubject<List<Budget>>();
  final BehaviorSubject<Map<int, double>> _budgetSpentController =
      BehaviorSubject<Map<int, double>>();
  ...
  Future<void> updateBudgetOnTransactionChange(
      Transaction transaction, TransactionChangeType changeType) async {
    ...
    try {
      ...
      for (final budget in affectedBudgets) {
        ...
        final shouldInclude =
            await _filterService.shouldIncludeTransaction(budget, transaction);
      ...
      if (affectedBudgets.isNotEmpty) {
        ...
        final currentSpentAmounts =
            _budgetSpentController.value ?? <int, double>{};
        for (final budget in affectedBudgets) {
          if (budget.id != null) {
  ...
  Future<void> _updateBudgetSpentAmount(Budget budget, Transaction transaction,
      TransactionChangeType changeType) async {
    double amountChange = 0.0;

    switch (changeType) {
      case TransactionChangeType.created:
        amountChange =
            transaction.amount.abs(); // Always positive for budget spending
        break;
    ...
    if (budget.normalizeToCurrency != null) {
      ...
      amountChange = await _filterService.normalizeAmountToCurrency(
          amountChange, transactionCurrency, budget.normalizeToCurrency!);
    }

    // Update budget spent amount
    final newSpentAmount =
        (budget.spent + amountChange).clamp(0.0, double.infinity);
