...
class GoogleDriveSyncService implements SyncService {
  ...
  Future<File> _exportDatabase() async {
    ...
    final tempFile = File(p.join(tempDir.path,
        'export_${DateTime.now().millisecondsSinceEpoch}.sqlite'));
  ...
  Future<void> _mergeDatabase(Stream<List<int>> dataStream) async {
    ...
    final tempFile = File(p.join(
        tempDir.path, 'merge_${DateTime.now().millisecondsSinceEpoch}.sqlite'));
    ...
    try {
      ...
      final remoteTxns =
          await tempDatabase.select(tempDatabase.transactionsTable).get();
      final remoteCategories =
          await tempDatabase.select(tempDatabase.categoriesTable).get();
      final remoteAccounts =
          await tempDatabase.select(tempDatabase.accountsTable).get();
      final remoteBudgets =
          await tempDatabase.select(tempDatabase.budgetsTable).get();
      final remoteAttachments =
          await tempDatabase.select(tempDatabase.attachmentsTable).get();
  ...
  Future<void> _mergeTransactions(
      List<TransactionsTableData> remoteTxns) async {
        ...
        await _database.into(_database.transactionsTable).insert(
              TransactionsTableCompanion.insert(
                ...
                createdAnotherFutureTransaction:
                    Value(remoteTxn.createdAnotherFutureTransaction),
  ...
  Future<void> _mergeCategories(
      List<CategoriesTableData> remoteCategories) async {
        ...
        await _database.into(_database.budgetsTable).insert(
              BudgetsTableCompanion.insert(
                ...
                budgetTransactionFilters:
                    Value(remoteBudget.budgetTransactionFilters),
                excludeDebtCreditInstallments:
                    Value(remoteBudget.excludeDebtCreditInstallments),
                excludeObjectiveInstallments:
                    Value(remoteBudget.excludeObjectiveInstallments),
                walletFks: Value(remoteBudget.walletFks),
                currencyFks: Value(remoteBudget.currencyFks),
                sharedReferenceBudgetPk:
                    Value(remoteBudget.sharedReferenceBudgetPk),
                ...
                includeTransferInOutWithSameCurrency:
                    Value(remoteBudget.includeTransferInOutWithSameCurrency),
                includeUpcomingTransactionFromBudget:
                    Value(remoteBudget.includeUpcomingTransactionFromBudget),
  ...
  Future<void> _mergeAttachments(
      List<AttachmentsTableData> remoteAttachments) async {
        ...
        await _database.into(_database.attachmentsTable).insert(
              AttachmentsTableCompanion.insert(
                ...
                isCapturedFromCamera:
                    Value(remoteAttachment.isCapturedFromCamera),
  ...
  Future<void> _markAllAsSynced() async {
    ...
    try {
      ...
      print(
          'Warning: Could not mark events as synced (event log may not exist): $e');
      await _updateLastSyncTime(now);
    }
  }
  ...
  Future<void> _markEventsAsSynced(List<String> eventIds) async {
    ...
    try {
      ...
      print(
          'Warning: Batch event marking failed, trying individual fallback: $e');
      ...
      print(
          '✅ PHASE 4.4: Individual fallback completed: $successCount/${eventIds.length} events marked');
    }
  }
