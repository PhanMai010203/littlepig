...
class TimerTask {
  ...
  void markFailed() {
    ...
    final backoffSeconds =
        min(pow(2, consecutiveFailures).toInt() * 60, 3600); // Max 1 hour
...
class TimerManagementService with WidgetsBindingObserver {
  ...
  static TimerManagementService get instance =>
      _instance ??= TimerManagementService._();
  ...
  Future<void> initialize() async {
    ...
    try {
      ...
      _isInitialized = true;
      debugPrint(
          'TimerManagementService initialized with battery level: $_lastBatteryLevel%');
    } catch (e) {
  ...
  void startMasterTimer() {
    ...
    debugPrint(
        'Master timer started with ${_masterInterval.inMinutes}min interval');
  }
  ...
  void registerTask(TimerTask task) {
    ...
    _tasks[task.id] = task;
    debugPrint(
        'Registered task: ${task.id} (${task.interval.inMinutes}min interval, priority: ${task.priority})');
  }
  ...
  List<TimerTask> _getTasksToExecute(DateTime now) {
    ...
    for (final task in _tasks.values) {
      ...
      if (_lastBatteryLevel <= _lowBatteryThreshold &&
          task.pauseOnLowBattery &&
          !task.isEssential) {
        continue;
      }

      // Check battery saver mode
      if (AppSettings.getWithDefault<bool>('batterySaver', false) &&
          !task.isEssential) {
        continue;
      }
  ...
  Future<void> _executeTask(TimerTask task) async {
    ...
    try {
      ...
      debugPrint(
          'Task ${task.id} failed: $e (failures: ${task.consecutiveFailures}, backoff: ${task.currentBackoff.inMinutes}min)');
    }
  }
  ...
  void _adjustTimerFrequency(bool isBackground) {
    if (isBackground) {
      ...
      stopMasterTimer();
      _masterTimer =
          Timer.periodic(const Duration(minutes: 5), _handleTimerTick);
  ...
  Map<String, dynamic> getPerformanceMetrics() {
    return {
      ...
      'executionTimes':
          _executionTimes.map((k, v) => MapEntry(k, v.inMilliseconds)),
