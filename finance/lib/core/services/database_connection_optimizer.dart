...
class DatabaseConnectionOptimizer {
  ...
  static Future<void> applyOptimizations(AppDatabase database) async {
    try {
      ...
      await database.customStatement('PRAGMA synchronous = NORMAL'); // Faster than FULL
      await database.customStatement('PRAGMA cache_size = -64000'); // 64MB cache
      await database.customStatement('PRAGMA temp_store = MEMORY'); // Use memory for temp tables
      ...
      // Memory management
      await database.customStatement('PRAGMA mmap_size = 268435456'); // 256MB mmap
  ...
  static DatabaseConnection createOptimizedConnection(String dbPath) {
    ...
    _sharedConnection = DatabaseConnection.fromExecutor(
      NativeDatabase.opened(database)
    );
  ...
  static void _applySqlite3Optimizations(Database database) {
    try {
      ...
      database.execute('PRAGMA locking_mode = NORMAL'); // Allow multiple connections
  ...
  static Future<Map<String, dynamic>> getPerformanceMetrics(AppDatabase database) async {
    try {
      final journalMode = await database.customSelect('PRAGMA journal_mode').getSingle();
      final cacheSize = await database.customSelect('PRAGMA cache_size').getSingle();
      final synchronous = await database.customSelect('PRAGMA synchronous').getSingle();
      final tempStore = await database.customSelect('PRAGMA temp_store').getSingle();
