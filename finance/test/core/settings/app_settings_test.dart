  ...
  group('AppSettings Animation Enhancement Tests', () {
    ...
    group('Default Animation Settings', () {
      ...
      test('should return defaults when settings are not set', () {
        expect(AppSettings.getWithDefault<String>('animationLevel', 'fallback'), equals('normal'));
        expect(AppSettings.getWithDefault<bool>('batterySaver', true), isFalse);
        expect(AppSettings.getWithDefault<bool>('appAnimations', false), isTrue);
      });
    });
    ...
    group('Setting Validation', () {
      ...
      test('should handle type mismatches gracefully', () {
        ...
        expect(AppSettings.getWithDefault<bool>('animationLevel', true), isTrue);
        expect(AppSettings.getWithDefault<String>('batterySaver', 'fallback'), equals('fallback'));
      });
    });

    group('Legacy Compatibility', () {
      test('should maintain compatibility with existing theme settings', () async {
      ...
      test('should work with existing reduceAnimations setting', () async {
        ...
        expect(AppSettings.reduceAnimations, isTrue);
        expect(AppSettings.getWithDefault<bool>('reduceAnimations', false), isTrue);
      });
    });
    ...
    group('Performance', () {
      ...
      test('should handle multiple rapid setting changes', () async {
        for (int i = 0; i < 10; i++) {
          await AppSettings.setAnimationLevel(i % 2 == 0 ? 'normal' : 'enhanced');
        }
