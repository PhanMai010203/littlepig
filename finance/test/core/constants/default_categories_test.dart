  ...
  group('Default Categories Tests', () {
    ...
    test('should have emoji icons for all categories', () {
      for (final category in DefaultCategories.allCategories) {
        ...
        expect(category.emoji.length,
            lessThanOrEqualTo(4)); // Allow for complex emojis
    ...
    test('should have unique sync IDs', () {
      final syncIds =
          DefaultCategories.allCategories.map((c) => c.syncId).toList();
    ...
    test('should have correct income and expense categories', () {
      final incomeCategories =
          DefaultCategories.allCategories.where((c) => !c.isExpense).toList();
      ...
      expect(
          incomeCategories
              .any((c) => c.name.contains('Lương') && c.emoji == '💰'),
          true);
      expect(
          expenseCategories
              .any((c) => c.name.contains('Ăn uống') && c.emoji == '🍽️'),
          true);
    });
    ...
    test('should have Vietnamese and English names', () {
      ...
      for (final category in DefaultCategories.allCategories) {
        expect(category.name.contains(' - '), true,
            reason:
                'Category "${category.name}" should have Vietnamese - English format');
      }
    });
  });
}
