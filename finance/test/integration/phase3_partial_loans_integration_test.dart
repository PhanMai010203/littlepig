  ...
  group('Phase 3: Partial Loans Integration Tests', () {
    ...
    group('Database Migration Tests', () {
      ...
      test('should initialize existing loan transactions correctly', () async {
        ...
        final creditResult = await database
            .customSelect(
                "SELECT * FROM transactions WHERE sync_id = 'old-credit-1'")
            .getSingle();

        expect(creditResult.data['remaining_amount'],
            equals(1000.0)); // abs(-1000)
        expect(
            creditResult.data['transaction_state'], equals('actionRequired'));

        // Assert: Check debt transaction
        final debtResult = await database
            .customSelect(
    ...
    group('End-to-End Loan Payment Workflows', () {
      test('should handle complete credit collection workflow', () async {
        ...
        expect(
            finalCredit.transactionState, equals(TransactionState.completed));
      ...
      test('should handle complete debt settlement workflow', () async {
        ...
        for (final settlement in settlements) {
          ...
          expect(
              settlement.transactionState, equals(TransactionState.completed));
    ...
    group('getRemainingAmount Helper Integration', () {
      test('should return correct remaining amounts throughout loan lifecycle',
          () async {
    ...
    group('Data Integrity Tests', () {
      test('should maintain parent-child relationships through repository',
          () async {
        // Arrange
        final credit = Transaction(
        ...
        await repository.collectPartialCredit(
            credit: createdCredit, amount: 200.0);

        final afterFirst =
            await repository.getTransactionById(createdCredit.id!);
        await repository.collectPartialCredit(
            credit: afterFirst!, amount: 300.0);
        ...
        expect(children.map((c) => c.amount).toList(),
            containsAll([200.0, 300.0]));
