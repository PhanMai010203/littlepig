...
void main() {
  group('Database Cache Performance Tests', () {
    ...
    test('Cache Invalidation Performance', () async {
      ...
      print(
          'Read after invalidation: ${stopwatchAfterInvalidation.elapsedMilliseconds}ms');
    ...
    test('Memory Usage and Cache Size', () async {
      ...
      expect(
          cacheStats['total_entries'], lessThan(100)); // Reasonable upper bound
    });

    test('Database Connection Optimization Verification', () async {
      // Get database performance metrics
      final metrics =
          await DatabaseConnectionOptimizer.getPerformanceMetrics(database);
      ...
      expect(
          metrics['journal_mode'], equals('wal')); // WAL mode should be enabled
      expect(
          metrics['foreign_keys'], equals(1)); // Foreign keys should be enabled
    });
  });
}
...
Future<Map<String, dynamic>> _getCacheStatistics() async {
  ...
  return {
    'total_entries':
        0, // This would need to be implemented in DatabaseCacheService
