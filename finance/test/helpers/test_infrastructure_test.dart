  ...
  group('Phase 4.3: Test Infrastructure Tests', () {
    group('TestEntityBuilders', () {
      ...
      test('should create transaction batch with correct count', () {
        final transactions = TestEntityBuilders.createTestTransactionBatch(count: 3);
    ...
    group('EventSourcingTestHelpers', () {
      ...
      test('should create transaction lifecycle events', () {
        final events = EventSourcingTestHelpers.createTransactionLifecycleEvents(
    ...
    group('TestDatabaseSetup', () {
      ...
      test('should create populated test database', () async {
        ...
        try {
          ...
          expect(categories.length, greaterThanOrEqualTo(6)); // 2 default + 4 additional
          expect(accounts.length, greaterThanOrEqualTo(4)); // 1 default + 3 additional
      ...
      test('should verify event sourcing triggers exist', () async {
        ...
        try {
          ...
          final createEvent = events.where((e) => 
    ...
    group('Integration Tests', () {
      test('should work together: entities, events, and database', () async {
        ...
        try {
          ...
          final isTransactionEventCreated = await EventSourcingTestHelpers.validateTriggersCreatedEvents(
            database,
            tableName: 'transactions',
            operation: 'create',
          ...
          final isAccountEventCreated = await EventSourcingTestHelpers.validateTriggersCreatedEvents(
            database,
            tableName: 'accounts',
            operation: 'create',
          ...
          // Get unsynced events
          final unsyncedEvents = await EventSourcingTestHelpers.getUnsyncedEvents(database);
          expect(unsyncedEvents.length, greaterThanOrEqualTo(2));
          
          // Cleanup test events
          await EventSourcingTestHelpers.cleanupTestEvents(database);
          
          final eventsAfterCleanup = await database.select(database.syncEventLogTable).get();
