...
class TestDatabaseSetup {
  ...
  static Future<void> _verifyEventSourcingTables(AppDatabase database) async {
    ...
    final eventTableInfo = await database.customSelect(
    ...
    final requiredEventColumns = [
      'id', 'event_id', 'device_id', 'table_name_field', 'record_id',
      'operation', 'data', 'timestamp', 'sequence_number', 'hash', 'is_synced'
    ];
    ...
    final stateTableInfo = await database.customSelect(
    ...
    final requiredStateColumns = [
      'id', 'device_id', 'last_sync_time', 'last_sequence_number', 'status'
    ];
  ...
  static Future<void> verifyPhase4TableStructure(AppDatabase database) async {
    final tables = ['transactions', 'accounts', 'categories', 'budgets', 'attachments'];
    ...
    for (final tableName in tables) {
      ...
      final columns = tableInfo.map((row) => row.data['name'] as String).toList();
      ...
      const legacyFields = ['device_id', 'is_synced', 'last_sync_at', 'version'];
      for (final legacyField in legacyFields) {
        expect(columns, isNot(contains(legacyField)),
          reason: '$tableName should not have legacy sync field: $legacyField');
      }
    }
  }
  ...
  static Future<void> verifyEventSourcingTriggers(AppDatabase database) async {
    ...
    for (final tableName in tables) {
      ...
      final insertTrigger = await database.customSelect(
      ...
      final deleteTrigger = await database.customSelect(
  ...
  static Future<void> _insertAdditionalTestCategories(AppDatabase database) async {
    final now = DateTime.now();
    ...
    for (int i = 0; i < additionalCategories.length; i++) {
      ...
      await database.into(database.categoriesTable).insert(
        CategoriesTableCompanion.insert(
          ...
          createdAt: Value(now),
          updatedAt: Value(now),
          syncId: 'test-cat-${isExpense ? 'expense' : 'income'}-${name.toLowerCase()}',
        ),
      );
    }
  }
  ...
  static Future<void> _insertAdditionalTestAccounts(AppDatabase database) async {
    final now = DateTime.now();
  ...
  static Future<void> _insertTestTransactions(AppDatabase database) async {
    ...
    final transactions = [
      ('Salary Payment', 3000.0, 1, 1), // Income: Salary category, default account
      ('Grocery Shopping', -75.0, 2, 1), // Expense: Food category, default account
      ('Gas Station', -45.0, 3, 1), // Expense: Transport category, default account
      ('Movie Night', -25.0, 4, 1), // Expense: Entertainment category, default account
      ('Stock Investment', -500.0, 6, 2), // Investment: Investment category, savings account
    ];
  ...
  static Future<void> verifyTestDataIntegrity(AppDatabase database) async {
    ...
    expect(defaultAccount, isNotNull,
      reason: 'Should have a default account');
  }
